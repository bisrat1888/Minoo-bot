...
// If the user sends a command (e.g. /setprice) while stuck mid-wizard,
// leave the scene instead of swallowing it as order input.
async function escapeIfCommand(ctx) {
  if (ctx.message?.text?.startsWith("/")) {
    await ctx.scene.leave();
    await ctx.reply("ትዕዛዙ ተቋርጧል። አሁን የላክከውን command እንደገና ላክ።", mainMenu());
    return true;
  }
  return false;
}
...
const orderWizard = new Scenes.WizardScene(
  ...
  async (ctx) => {
    if (await escapeIfCommand(ctx)) return;
...
bot.command("cancel", async (ctx) => {
  ...
  return ctx.reply("ተቋርጧል።", mainMenu());
});
